 ...
 *
 * HIP Port of subscriber.cuh
 * Target: AMD MI450 (gfx1250)
 *
 * Changes from CUDA version:
 * - Replaced <cuda/atomic> with HIP compatibility headers
 * - Replaced <nvshmem.h> with nvshmem_compat.hpp (rocSHMEM mappings)
 * - Updated WARP_SIZE to use flashmoe::WARP_SIZE from constants.hpp (32 on MI450/gfx1250)
 * - Replaced __shfl_sync with HIP-compatible version from cuda_compat.hpp
 * - Replaced __syncwarp with __builtin_amdgcn_wave_barrier()
 * - Replaced cuda::ptx::st with volatile store + memory fence
 * - cuda::atomic_ref uses HIP implementation from cuda_compat.hpp
 *
 * Note: MI450/gfx1250 is wave32.
 */

#ifndef FLASHMOE_HIP_SUBSCRIBER_CUH
#define FLASHMOE_HIP_SUBSCRIBER_CUH
...
// Infrastructure headers (will need HIP ports or are platform-agnostic)
#include "flashmoe/hip/bitset.hip.cuh"
#include "flashmoe/infra/dq.cuh"
...
// Use WARP_SIZE from flashmoe namespace.
using flashmoe::WARP_SIZE;

// ============================================================================
// HIP-specific PTX Store Replacement
// ============================================================================

namespace detail {

/**
 * hip_store_global - Replacement for cuda::ptx::st(cuda::ptx::space_global, ...)
 *
 * On AMD GPUs, we use a volatile store followed by a memory fence to ensure
 * visibility across wavefronts. This achieves similar semantics to the CUDA
 * PTX global store instruction.
 *
 * For aligned 16-byte stores (Ingredients is 16-byte aligned), this compiles
 * to efficient global_store_dwordx4 instructions on AMD.
 */
template<typename T>
__device__ __forceinline__
void hip_store_global(T* __restrict__ dest, const T& value) {
    static_assert(std::is_trivially_copyable_v<T>, "Type must be trivially copyable");

    __builtin_memcpy(dest, &value, sizeof(T));
    __threadfence();
}
...
// ============================================================================
// HIP-compatible Warp Synchronization
// ============================================================================

/**
 * warp_sync - Wavefront synchronization barrier
 *
 * On AMD, wavefronts execute in lockstep so this is primarily a memory barrier.
 * Uses the AMD-specific wave barrier intrinsic.
 */
__device__ __forceinline__
void warp_sync() {
...
__device__ __forceinline__
bool expectedState(const uint8_t currentSense, const uint8_t receivedSense,
                   const uint8_t currentState, const uint8_t receivedState) {
...
struct Decoder {
    ...
    void operator()(const Args& args,
        ...
        std::array<cuda::std::byte*, GEMMs> taskResults{};
...
struct Decoder<subscriberCount, PacketStage::last, PeerConnectivity::p2p> {
    ...
    void operator()(const Args& args, const Ingredients& ingredients, unsigned int& lTQHead) const {
        ...
        // Note: We use hip_store_global as replacement for cuda::ptx::st
        // This ensures proper memory ordering and visibility across wavefronts
        auto* tQp = &((args.tQ + DQ::sNext<DQType::stride, subscriberCount>(lTQHead++))->ingredients);
        detail::hip_store_global(tQp, ingredients);
...
void sTB(const Args& args, const int& peer, const int& peerTaskTiles = 0) {
    if (!flashmoe::atomicTAS<cuda::thread_scope_block>(args.status + peer)) {
...
struct Subscriber {
    ...
    void operator()(const Heap& symHeap,
        ...
        for (int i = 0; i < stageLength; ++i) {
            ...
            if (laneId == 0) {
                ...
                if (!visitedSet.get(vIdx)) {
                    if (topo == Topology::MIXED && pLI.isRemote) {
                        // RDMA peer - use rocSHMEM signal operations
                        ...
                        // NVLink peer (or AMD Infinity Fabric peer)
                        cuda::atomic_ref<uint64_t, cuda::thread_scope_system> f{*(flags + flagIdx)};
                        signal = f.load(cuda::memory_order_acquire);
                        const auto sigPayload = cuda::std::bit_cast<SignalPayload<PacketStage::initial>>(signal);
                        if (sigPayload.stateNumber == currentStateNumber) {
                            sTB(args, peerIdx, sigPayload.totalTilesM);
                            visitedSet.set(vIdx);
            ...
            // HIP: Use wave barrier instead of __syncwarp
            warp_sync();

            // broadcast received signal from leader to others
            signal = __shfl_sync(0xffffffffffffffff, signal, 0);
            const auto sigPayload = cuda::std::bit_cast<SignalPayload<PacketStage::initial>>(signal);
...
struct Subscriber<SubscriberStage::final, Topology::NVLINK_ONLY, Element, subscriberCount, bM> {
    // Every peer is P2P-connected (NVLink or AMD Infinity Fabric)
    __device__ __forceinline__
    void operator()(const Args& args, BitSet* __restrict__ const& bitSet,
                    uint64_t* __restrict__ const& flags,
                    uint& ltQHead, const uint& stageLength) const {
        constexpr int wSet = 16;
        constexpr int bSw = sizeof(uint) * 8U;
        ...
        if (const auto residue = stageLength - stageTrips * wSet; residue) {
            ...
            for (uint j = 0; j < residue; ++j) {
                ...
                if (!sBS.get(bIdx)) {
                    cuda::atomic_ref<uint64_t, cuda::thread_scope_system> f{*(flags + flagIdx)};
                    const auto signal = f.load(cuda::memory_order_acquire);
                    const auto sigPayload = cuda::std::bit_cast<SignalPayload<PacketStage::last>>(signal);
                    const auto expected = expectedState(senseBitSet.get(bIdx),
                                                        sigPayload.senseBit, args.stateNumber,
                                                        sigPayload.stateNumber);
...
void start(const Heap& symHeap, const Args& args, const uint& firstStageBitSetLength) {
    ...
    // HIP ignores the mask argument and executes the active gfx1250 lanes.
    interrupt = __shfl_sync(0xffffffffffffffff, interrupt, 0);
...
#endif  // FLASHMOE_HIP_SUBSCRIBER_CUH
